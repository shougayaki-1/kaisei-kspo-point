...
function open(name = `court-result-${crypto.randomUUID()}`): AppDatabase { names.add(name); const db = createDatabase(name); databases.push(db); return db }
afterEach(async () => { for (const db of databases.splice(0)) db.close(); for (const name of names) { const db = createDatabase(name); await db.delete() } names.clear() })

const ids = {
  tournament: 'tournament-1' as TournamentId, competition: 'competition-1' as CompetitionId,
  slot: 'slot-1' as ScheduleSlotId, session: 'session-1' as ScoringSessionId,
...
interface ExpectedCourtResultService {
  ...
  loadSession(scoringSessionId: ScoringSessionId): Promise<{ session: { scoringSessionId: ScoringSessionId; courtRunIds: CourtRunId[] }; inputSchema: { inputSchemaId: string; version: number; fields: Array<{ key: string; label: string }> }; courtRuns: Array<{ courtRunId: CourtRunId }>; entries: Array<{ entryId: CompetitionEntryId; label: string }>; configVersion: number }>
  saveResult(input: { scoringSessionId: ScoringSessionId; courtRunIds?: CourtRunId[]; operator: string; inputMode: InputMode; values: Record<string, Record<string, unknown>> }): Promise<{ result: Result; revision: ResultRevision }>
  correctResult(input: { resultId: ResultId; operator: string; inputMode: InputMode; values: Record<string, Record<string, unknown>> }): Promise<{ result: Result; revision: ResultRevision }>
  listSessionResults(scoringSessionId: ScoringSessionId): Promise<Array<{ result: Result; revisions: ResultRevision[]; projection: ResultProjection }>>
  getResultHistory(resultId: ResultId): Promise<{ result: Result; revisions: ResultRevision[]; projection: ResultProjection }>
}
function service(db: AppDatabase): ExpectedCourtResultService { return createCourtResultService(db, { deviceId: 'court-device' as DeviceId }) as unknown as ExpectedCourtResultService }
function allValues(countA = '0.10', countB = '2') { return { [ids.entryA]: { count: countA, verified: true }, [ids.entryB]: { count: countB, verified: true } } }
...
describe('Court production Result service', () => {
  ...
  it('creates only raw Result + initial immutable Revision and canonicalizes decimal input', async () => {
    const db = open(); await seed(db); const target = service(db)
    const saved = await target.saveResult({ scoringSessionId: ids.session, operator: '担当者', inputMode: 'NUMBER', values: allValues() })
    expect(saved.result).toMatchObject({ tournamentId: ids.tournament, competitionId: ids.competition, scoringSessionId: ids.session, currentRevisionId: saved.revision.revisionId })
    expect(saved.revision).toMatchObject({ revisionNumber: 1, parentRevisionIds: [], source: 'COURT', operator: '担当者', inputMode: 'NUMBER', configVersion: 1 })
    expect(saved.revision.rawData).toEqual({ inputSchemaId: 'schema-1', inputSchemaVersion: 1, courtRunIds: [ids.runA, ids.runB], entries: { [ids.entryA]: { count: '0.1', verified: true }, [ids.entryB]: { count: '2', verified: true } } })
    expect(JSON.stringify(saved.revision.rawData)).not.toMatch(/award|points|rank|standings/i); expect(await db.transferBatches.count()).toBe(0)
  })
  it('applies InputSchema production validation and rejects unknown CompetitionEntry data', async () => {
    const db = open(); await seed(db); const target = service(db)
    await expect(target.saveResult({ scoringSessionId: ids.session, operator: '担当者', inputMode: 'NUMBER', values: { [ids.entryA]: { count: '101', verified: true }, [ids.entryB]: { count: '2', verified: true } } })).rejects.toThrow(/count|個数|max|100|range/i)
    await expect(target.saveResult({ scoringSessionId: ids.session, operator: '担当者', inputMode: 'NUMBER', values: { ...allValues(), ['unknown-entry']: { count: '3', verified: true } } })).rejects.toThrow(/entry|CompetitionEntry|unknown/i)
  ...
  it('creates corrections as child revisions and preserves the previous revision unchanged', async () => {
    const db = open(); await seed(db); const target = service(db)
    const first = await target.saveResult({ scoringSessionId: ids.session, operator: '担当者A', inputMode: 'NUMBER', values: allValues('1', '2') }); const firstSnapshot = structuredClone(first.revision)
