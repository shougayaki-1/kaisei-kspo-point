...
export function App({ confirmReload = (message) => window.confirm(message), reload = () => window.location.reload(), configRepository, operatorName = '本部担当', pwaRuntime, resetPersistentData, hostBackupServices }: AppProps = {}) {
  ...
  const [pwaSnapshot, setPwaSnapshot] = useState<PwaRuntimeSnapshot>(() => pwaRuntime?.getSnapshot() ?? { ...UNSUPPORTED_PWA_SNAPSHOT })
  ...
  const browserHostBackupServices = useMemo<HostBackupPanelServices>(() => ({ createBackup: () => createHostBackup(appDatabase), prepareRestore: (json) => prepareHostRestore(json), restore: (prepared) => restorePreparedHostBackup(appDatabase, prepared) }), [appDatabase])
  ...
  useEffect(() => { let cancelled = false; appDatabase.open().then(() => { if (!cancelled) setStorageAvailable(true) }).catch(() => { if (!cancelled) setStorageAvailable(false) }); return () => { cancelled = true } }, [appDatabase])
  useEffect(() => { if (!pwaRuntime) { setPwaSnapshot({ ...UNSUPPORTED_PWA_SNAPSHOT }); return }; setPwaSnapshot(pwaRuntime.getSnapshot()); return pwaRuntime.subscribe(setPwaSnapshot) }, [pwaRuntime])
  useEffect(() => { if (configRepository) return; let cancelled = false; appDatabase.tournaments.toCollection().first().then(async (tournament) => { if (cancelled || !tournament) return; setActiveTournamentId(tournament.tournamentId); setKnownConfigVersion(tournament.currentConfigVersion); const active = await browserConfigRepository.getActiveVersion(tournament.tournamentId); if (!cancelled) setKnownConfigVersionId(active?.configVersionId ?? null) }).catch(() => {}); return () => { cancelled = true } }, [appDatabase, browserConfigRepository, configRepository])
  ...
  if (mode === 'HOST') {
    ...
    content = <><div className="mode-header"><div><h1>表示モード</h1><p>本部の統合済み得点・順位を読み取り専用で表示します。</p></div><button type="button" onClick={returnToModeSelection}>モード選択へ戻る</button></div><DisplayDashboard service={hostScoringServices} /></>
  } else {
    content = <><h1>開成運動交流祭 得点管理</h1><p>使用するモードを選択してください。</p><div className="mode-actions"><button type="button" onClick={() => { setHostTab('CONFIG'); setMode('HOST') }}>本部モード</button><button type="button" onClick={() => setMode('COURT')}>コートモード</button><button type="button" onClick={() => setMode('DISPLAY')}>表示モード</button></div></>
  }

  return <div className="app-shell"><main>{content}</main><DeviceDiagnostics appVersion={APP_VERSION} activeConfigVersionId={knownConfigVersionId} storageAvailable={storageAvailable} pwa={pwaSnapshot} onActivateUpdate={pwaRuntime && mode !== 'DISPLAY' ? handleActivateUpdate : undefined} />{mode !== 'DISPLAY' ? <DataManagementPanel onReset={handleResetPersistentData} /> : null}<footer className="status-bar" aria-label="端末状態"><span>App {APP_VERSION}</span><span>{knownConfigVersion === null ? 'Config -' : `Config v${knownConfigVersion}`}</span><span>Device {deviceId.slice(0, 8)}</span><button type="button" className="reload-button" onClick={handleReload}>アプリを再読み込み</button></footer></div>
}
