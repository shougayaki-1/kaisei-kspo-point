...
describe('authoritative aggregate standings', () => {
  ...
  it('serializes identically for the same authoritative inputs regardless of event-score query order', () => {
    ...
    const first = buildAggregateStandings(teams, scores)
    const second = buildAggregateStandings([...teams].reverse(), [...scores].reverse())
    expect(serializeStandings(first)).toBe(serializeStandings(second))
  })
})
