...
import type { CompetitionEntryId, CompetitionId, CourtRunId, ResultId, RevisionId, ScoringSessionId } from '../domain/ids'
...
function result(): Result { return { resultId: 'result-1' as ResultId, tournamentId: 'tournament-1' as never, competitionId: 'competition-1' as CompetitionId, scoringSessionId: sessionId, currentRevisionId: 'rev-2' as RevisionId, createdAt: '2026-08-19T10:00:00+09:00', createdByDeviceId: 'court-device' as never } }
function revision(id: string, number: number, countA: string): ResultRevision { return { revisionId: id as RevisionId, resultId: 'result-1' as ResultId, revisionNumber: number, parentRevisionIds: number === 1 ? [] : ['rev-1' as RevisionId], source: 'COURT', operator: number === 1 ? '担当者A' : '担当者B', inputMode: 'NUMBER', rawData: { inputSchemaId: 'schema-1', inputSchemaVersion: 1, courtRunIds: [runA, runB], entries: { [entryA]: { count: countA, verified: true }, [entryB]: { count: '2', verified: true } } }, configVersion: 1, createdAt: `2026-08-19T10:0${number}:00+09:00` } }
function services() {
  const savedResult = result(); const revisions = [revision('rev-1', 1, '1'), revision('rev-2', 2, '3')]
  return {
    listSessions: vi.fn().mockResolvedValue([{ scoringSessionId: sessionId, label: '第1展開 全体', competitionName: '計数競技', inputScope: 'WHOLE_SLOT', courtRunCount: 2 }]),
...
describe('CourtScoringSession production UI', () => {
  ...
  it('submits raw production input to the application service without calculating points in React', async () => {
    ...
    fireEvent.change(screen.getByLabelText('赤A 個数'), { target: { value: '0.10' } }); fireEvent.change(screen.getByLabelText('青B 個数'), { target: { value: '2' } }); fireEvent.click(screen.getByLabelText('赤A 確認')); fireEvent.click(screen.getByLabelText('青B 確認')); fireEvent.change(screen.getByLabelText('担当者'), { target: { value: 'コート担当' } }); fireEvent.click(screen.getByRole('button', { name: '結果を保存' }))
